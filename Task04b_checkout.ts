...
// checkout.addDiscountCodeToCart(cartId, "TULIPS_BOGO").then(log).catch(log);
...
// checkout.updateOrderCustomState(orderId, "ll-order-packed").then(log).catch(log);
...
const checkoutProcess = async () => {
    ...
    filledCart = await checkout.addDiscountCodeToCart(
        filledCart.body.id, "TULIPS_BOGO"
    );
    ...
    order = await checkout.updateOrderCustomState(order.body.id, "ll-order-packed");
    if (order) {
        return {
            status: 201,
